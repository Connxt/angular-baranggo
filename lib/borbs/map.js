var map = function() {
	...
	outdoor.eachLayer( function (locale) {
		...
		var lat = coor[1];
		var lng = coor[0];
		var content = '<h3>' + family + '<br/></h3><div class="tooltip"><p>' + street + ' Brgy. ' + brgy + ' ' + city + '<br/> <strong>Member </strong>:'  + memar + '</p></div>';
		var m = residencialHouses.addLayer(L.marker([lat,lng])
					.bindPopup(content)
				).addTo(map);
		m.addTo(map);
	});
		...
		var filter = L.geoJson(geojson, {
			filter: function ( feature, layer) {
				...
				if(index !== -1){
					...
					// $("#purok").text(feature.properties.purok);
					var content = '<h3>' + family + '</h3><div class="tooltip"><p>' + street + ' Brgy. ' + brgy + ' ' + city + '</p></div>';
